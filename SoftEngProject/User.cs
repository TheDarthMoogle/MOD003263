...
namespace SoftEngProject
{
    public partial class User : Form
    {
        ...
        public TemplateList templateList = new TemplateList(0);
        ...
        private void BuildTemplates()
        {
            ...
            {
                int tID = row.Field<Int32>(0);
                string tName = row.Field<string>(1);
                Template newTemplate = new Template(tID, tName, "", null);
        ...
        private void BuildToolStripTemplateItems()
        {
            templateToolStripMenuItem1.DropDownItems.Clear();

            foreach (Template t in templateList)
            {
                ToolStripMenuItem item = new ToolStripMenuItem();
                item.Name = t.TemplateName + "ToolStripMenuItem";
                item.Tag = t.ID;
                item.Text = t.TemplateName;
                item.Click += new EventHandler(MenuItemClickHandler);
                templateToolStripMenuItem1.DropDownItems.Add(item);
            }
        }
        /// <summary>
        ...
        private void MenuItemClickHandler(object sender, EventArgs e)
        {
            ...
            if (gbxList.Count > 0)
            {
                for (int i = 0; i < gbxList.Count; i++)
                {
                    GroupBox gb = gbxList[i];
                    gb.Controls.Clear();
                    gb.Dispose();
                }
                gbxList.Clear();
        ...
        private void newBtnUp_Clicked(object sender, EventArgs e)
        {
            ...
            if (parentBoxIndex > 0)
            {
                ...
                ComboBox cbx = cbxList[parentBoxIndex];
                cbxList.RemoveAt(parentBoxIndex);
                cbxList.Insert(groupBoxAboveIndex, cbx);
                TextBox tbx = tbxList[parentBoxIndex];
                tbxList.RemoveAt(parentBoxIndex);
                tbxList.Insert(groupBoxAboveIndex, tbx);
        ...
        private void newBtnDown_Clicked(object sender, EventArgs e)
        {
            ...
            if (parentBoxIndex < (gbxList.Count - 1))
            {
                ...
                ComboBox cbx = cbxList[parentBoxIndex];
                cbxList.RemoveAt(parentBoxIndex);
                cbxList.Insert(groupBoxBelowIndex, cbx);
                TextBox tbx = tbxList[parentBoxIndex];
                tbxList.RemoveAt(parentBoxIndex);
                tbxList.Insert(groupBoxBelowIndex, tbx);
        ...
        {
            Control btnDel = sender as Control;
        ...
        private void newBtnEdit_Clicked(object sender, EventArgs e)
        {
            ...
            Response newResponse = new Response(i+1, cbx.Text, tbx.Text, _responseList.ID.ToString());
        ...
        private void templateToolStripMenuItem3_Click(object sender, EventArgs e)
        {
            ...
            using (NewTemplate tmpltForm = new NewTemplate())
            {
                if (tmpltForm.ShowDialog() == DialogResult.OK)
                {
                    Console.WriteLine("newTemplate Closed");
                    templateList.Add(tmpltForm.newTemplateCreated);
                }
            }
        }

        private void openToolStripMenuItem_DropDownOpened(object sender, EventArgs e)
        {
            BuildToolStripTemplateItems();
        }
    }
